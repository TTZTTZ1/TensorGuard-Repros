...
sparse_tensor.dense_tensor = torch.randn(3, 3)
sparse_tensor.dense_tensor.requires_grad = True
sparse_tensor.size = torch.Size([3, 1])
sparse_tensor.indices = torch.tensor([0, 1, 2], dtype=torch.int, device=torch.device('cpu'), requires_grad=False)
sparse_tensor.values = (2 * torch.ones(2))
